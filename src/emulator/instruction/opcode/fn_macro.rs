macro_rules! add_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<add_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
...
macro_rules! or_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<or_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! adc_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<adc_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! sbb_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<sbb_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
            let cf:  $type = exec.check_rflags_b()? as $type;
...
macro_rules! and_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<and_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! sub_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<sub_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! xor_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<xor_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! cmp_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<cmp_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! push_src {
    ( $type:ty, $src:ident ) => { paste::item! {
        fn [<push_ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
...
macro_rules! pop_dst {
    ( $type:ty, $dst:ident ) => { paste::item! {
        fn [<pop_ $dst>](exec: &mut exec::Exec) -> Result<(), OpError> {
...
macro_rules! imul_dst_src1_src2 {
    ( $type:ty, $dst:ident, $src1:ident, $src2:ident ) => { paste::item! {
        fn [<imul_ $dst _ $src1 _ $src2>](exec: &mut exec::Exec) -> Result<(), OpError> {
...
macro_rules! jcc_rel {
    ( $type:tt, $cc:ident, $rel:ident ) => {
        paste::item! {
            fn [<j $cc _ $rel>](exec: &mut exec::Exec) -> Result<(), OpError> {
                ...
                fn [<jn $cc _ $rel>](exec: &mut exec::Exec) -> Result<(), OpError> {
                    if(!exec.[<check_rflags_ $cc>]()?){
                    let rel: $type = exec.[<get_ $rel>]()? as $type;
                    debug!("jmp: {}", rel);
                    exec.update_rip(rel as i64)?;
...
macro_rules! test_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<test_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! xchg_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<xchg_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let dst: $type = exec.[<get_ $dst>]()?;
            let src: $type = exec.[<get_ $src>]()?;
...
macro_rules! mov_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<mov_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
            let src: $type = exec.[<get_ $src>]()?;
            debug!("mov: {:02x}", src);
            exec.[<set_ $dst>](src)?;
            Ok(())
...
macro_rules! lea_dst_src {
    ( $type:ty, $dst:ident, $src:ident ) => { paste::item! {
        fn [<lea_ $dst _ $src>](exec: &mut exec::Exec) -> Result<(), OpError> {
...
macro_rules! setcc_dst {
    ( $type:tt, $cc:ident, $dst:ident ) => {
        paste::item! {
            fn [<set $cc _ $dst>](exec: &mut exec::Exec) -> Result<(), OpError> {
                let flag: bool = exec.[<check_rflags_ $cc>]()?;
                exec.[<set_ $dst>](flag as $type)?;
                Ok(())
            }

            fn [<setn $cc _ $dst>](exec: &mut exec::Exec) -> Result<(), OpError> {
                let flag: bool = exec.[<check_rflags_ $cc>]()?;
                exec.[<set_ $dst>](!flag as $type)?;
                Ok(())
            }
